...
function downloadReport() {
  ...
  if (!probNum || probNum.textContent === '--') {
    showToast('Please run a prediction first!', 'error');
    return;
  }

  const prob = probNum.textContent;
  const risk = riskBadge.textContent;
  const recElements = document.querySelectorAll('.rec-item');
  let recsText = "";
  recElements.forEach(el => {
    recsText += `- ${el.querySelector('.rec-title').textContent}: ${el.querySelector('.rec-detail').textContent}\n`;
  });

  const content = `RETENTO.AI - CHURN ANALYSIS REPORT\n` +
                  `====================================\n` +
                  ...
                  `RETENTION PLAYBOOK:\n${recsText}\n` +
                  `====================================\n` +
                  `Generated by Retento.ai Intelligence Edge`;

  const blob = new Blob([content], { type: 'text/plain' });
  const url = URL.createObjectURL(blob);
  const a = document.createElement('a');
  a.href = url;
  a.download = `Retento_Report_${new Date().getTime()}.txt`;
...
// ── INIT ─────────────────────────────────────────────────────────
window.onload = () => {
  // Animate the record counter
  animateNumber('records-counter', 5042, ' Records');
};
